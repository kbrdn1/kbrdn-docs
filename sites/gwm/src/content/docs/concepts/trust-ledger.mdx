...
quiconque a rédigé le `.gwm.toml`. Le trust ledger (issue #95) établit une
frontière explicite entre « j'ai cloné ce remote » et « ce remote peut exécuter
des commandes en mon nom ».
...
ferait qu'entraîner l'utilisateur à marteler `y`.
...
`y` enregistre et continue, `N` (ou EOF / réponse non reconnue) annule, `show`
réaffiche le `.gwm.toml` brut. Les exécutions suivantes passent silencieusement
jusqu'à ce que le fichier change.
