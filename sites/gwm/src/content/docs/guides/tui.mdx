...
`.` (action `open_docs`, `:open-docs` dans la palette) ouvre la **documentation
en ligne de gwm** dans le navigateur. L'URL est **figée à la compilation** à
partir des métadonnées du paquet (`CARGO_PKG_REPOSITORY` + `/tree/main/docs`) :
elle est identique quel que soit le dépôt dans lequel vous lancez gwm — ce n'est
pas la doc du dépôt courant. (Un binaire compilé depuis un fork pointe vers la
doc de ce fork, puisque l'URL est gravée au build, pas résolue au runtime.)
