...
- `!` — mot-clé non reconnu (avertissement : la commande s'exécute quand même, en
  se rabattant sur `true`).
...
- `✓` — tous résolus.
- `!` — binaire `[review]` manquant (la review est opt-in).
- `✗` — binaire `[git_tui]` ou d'une commande de bootstrap manquant.
...
- `✗` — ni l'une ni l'autre (gwm ne peut pas créer de worktrees ici).
