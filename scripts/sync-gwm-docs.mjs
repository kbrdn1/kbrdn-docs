...
// Noyau pur (typographie, capitalisation, mapping, conversion) : extrait dans
// `lib/convert.mjs` pour être couvert par `bun test`. Ce fichier-ci garde ce qui
// touche au disque et l'ordonnancement.
import {
  convert,
  dedashProse,
...
// Les racines sont injectees ici : le noyau ne connait aucun chemin global.
const targetFor = (abs) => targetForIn(abs, { src: SRC, docs: DOCS });
...
const versionPubliee = versionFromCargoToml(cargo);
if (!versionPubliee) {
