...
First tagged version of the platform. Everything below shipped between the
initial scaffold and this tag: the Bun-workspaces monorepo, the shared design
system, the gwm site and its generation pipeline from `kbrdn1/gwm-cli`.
...
- `@kbrdn/ds-shared`: three surface levels instead of one flat wash — a sunken
  ...
  with a lighter border, so screenshots read as _on_ the page rather than _in_
